...
__global__ void process_pblock(uint8_t *Yonly, uint8_t *reconstructed, uint8_t *predicted_frame, uint8_t *motion_vector, int bf){
	...
	unsigned int local_sad [2*R-1][1];
  ...
  // ------------ non-first frame ------------
    ...
    //PPprintf("processing window_block br,bc = %u,%u ..... start end=%u,%u, %u,%u\n",br, bc, startr, endr,startc,endc);
    ...
    for (register int rr=startr; rr<endr; rr++)
      for (register int cc=startc; cc<endc; cc++){
		...
		printf("(%d,%d) computing sad for %d, %d -> %d\n", pr,pc,rr, cc,local_sad[rr-startr][cc-startc]);
	  }
