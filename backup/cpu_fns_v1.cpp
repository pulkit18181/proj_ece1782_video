...
int get_sad(uint8_t *Y1, int br1,int bc1,int bf1, uint8_t *Y2, int br2,int bc2,int bf2, bool print=0){
    int val = 0;
    int size2 = B;
    int idx1 = bf1*MN +getP(br1)*N +getP(bc1);
    int idx2 = bf2*MN +getP(br2)*N +getP(bc2);
    for (int ii=0; ii<B; ii++){
        for (int jj=0; jj<B; jj++){
            int idx1f = idx1 + ii*N + jj;
            int idx2f = idx2 + ii*size2 + jj;
...
void process_iblock(uint8_t *Yonly, uint8_t *reconstructed, uint8_t *predicted_frame, uint8_t *motion_vector, int br,int bc,int bf){
    ...
    int curr = bf*MN +getP(br)*N +getP(bc);
    int left = bf*MN +getP(br)*N +getP(bc-1);
    int top  = bf*MN +getP(br-1)*N +getP(bc);
